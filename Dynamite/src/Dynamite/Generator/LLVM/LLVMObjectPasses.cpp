...
namespace Dynamite
{
	...
	void LLVMObjectPasses::Run(llvm::Module& mod, const std::filesystem::path& objectPath)
	{
        ...
        if (!target)
        {
            CompilerSuite::Get().Error("Failed to find target, error: {0}", error);
            return;
        }
        ...
        if (errorCode)
        {
            CompilerSuite::Get().Error("Could not open file: '{0}'\n    Error: {1}", objectPath.string(), errorCode.message());
            return;
        }
        ...
        {
            CompilerSuite::Get().Error("TargetMachine can't emit a file of this type");
            return;
        }
